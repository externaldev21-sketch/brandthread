...
router.get("/", async (req, res) => {
  ...
  const rows = await db.select().from(ipCases).where(status ? eq(ipCases.status, status) : undefined)
...
router.patch("/:id", async (req, res) => {
  ...
  const [existing] = await db.select().from(ipCases).where(eq(ipCases.id, req.params.id)).limit(1);
  if (!existing) return res.status(404).json({ error: "Case not found" });
  const nextStatus = status ?? (action ? "actioned" : existing.status);
  if (action && !existing.listingProductId) return res.status(400).json({ error: "Case has no product listing target" });
  const now = new Date();
  await db.transaction(async (tx) => {
    if (action && existing.listingProductId) {
      await tx.update(products).set({ status: "archived", deletedAt: now, recoverableUntil: null, removalKind: "moderation_removed", updatedAt: now })
    ...
    await tx.insert(ipCaseAuditHistory).values({ caseId: existing.id, action: action ?? "status_updated",
      ...
      details: moderatorNotes !== undefined ? { moderatorNotesUpdated: true } : {} });
  });
