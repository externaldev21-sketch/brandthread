# Migrating Expo UI to Brandthread Woven

Read `artifacts/brandthread-woven/docs/AGENTS.md` and
`artifacts/brandthread-woven/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/brandthread-woven/lib/native-theme` and
  `@workspace/brandthread-woven/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/brandthread-woven/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/brandthread-woven` and retains only
