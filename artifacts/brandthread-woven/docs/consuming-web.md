# Consuming Brandthread Woven in web apps

Read `artifacts/brandthread-woven/docs/AGENTS.md` first. This guide covers
...
`artifacts/brandthread-woven/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/brandthread-woven/styles.css";
```
...
`node_modules/@workspace/brandthread-woven/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/brandthread-woven/components/ui/button";
import { cn } from "@workspace/brandthread-woven/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/brandthread-woven/hooks/use-toast";
```
...
`@workspace/brandthread-woven/components/ui/button`. Run the app's typecheck
