export default function Slide11SellerWorkspace() {
    ...
    <div className="w-screen h-screen overflow-hidden relative" style={{background:'#030503',fontFamily:"'Inter',sans-serif"}}>
      ...
      <div style={ph}>
        ...
        <div style={{padding:'1.5vh 3vh 0',display:'flex',gap:'0.6vh',flexShrink:0}}>
          {dot(true)}{dot(true)}{dot(true)}{dot(true)}{dot(true)}{dot(true)}{dot(true)}{dot(true)}{dot(true)}
        </div>
        <div style={{flex:1,overflow:'hidden',padding:'2.5vh 3vh 0',display:'flex',flexDirection:'column'}}>
          <div style={{fontSize:'2.8vh',fontWeight:800,color:'#FFF',letterSpacing:'-0.03em',marginBottom:'0.5vh'}}>What do you want to see first?</div>
          <div style={{fontSize:'1.2vh',color:'rgba(255,255,255,0.45)',marginBottom:'2.5vh'}}>Your workspace will open here by default.</div>
          {/* Options */}
          {[
            {icon:'🎨',label:'Design studio',sel:false},
            {icon:'🏭',label:'Manufacturer search',sel:true},
            {icon:'🛍️',label:'My storefront',sel:false},
            {icon:'📦',label:'Order management',sel:false},
            {icon:'📊',label:'Analytics',sel:false},
          ].map((m) => (
            <div key={m.label} style={{background: m.sel ? 'rgba(74,144,226,0.08)' : '#171717',borderRadius:'1.2vh',border: m.sel ? '0.2vh solid #4A90E2' : '0.15vh solid #2A2A2A',padding:'1.3vh 1.5vh',marginBottom:'0.9vh',display:'flex',alignItems:'center',gap:'1.2vh'}}>
              <span style={{fontSize:'1.8vh'}}>{m.icon}</span>
              <span style={{fontSize:'1.3vh',fontWeight:500,color: m.sel ? '#4A90E2' : 'rgba(255,255,255,0.75)',flex:1}}>{m.label}</span>
              {m.sel && <span style={{fontSize:'1.3vh',color:'#4A90E2'}}>✓</span>}
            </div>
          ))}
          <div style={{display:'flex',justifyContent:'flex-end',marginTop:'1.5vh'}}>
            <div style={{background:'#4A90E2',borderRadius:'1.4vh',padding:'1.3vh 2.5vh'}}>
              <span style={{fontSize:'1.3vh',fontWeight:700,color:'#FFF'}}>Build my workspace</span>
            </div>
          </div>
        </div>
      ...
      <div style={{position:'absolute',bottom:'3vh',left:'7vw',right:'7vw',display:'flex',justifyContent:'space-between'}}>
        <span style={{fontSize:'1.5vw',color:'rgba(255,255,255,0.3)',fontWeight:500,letterSpacing:'0.04em'}}>SELLER ONBOARDING — WORKSPACE (STEP 9)</span>
        <span style={{fontSize:'1.2vw',color:'rgba(255,255,255,0.18)'}}>11 / 19</span>
      </div>
    </div>
  );
}
