...
export function CookieConsentProvider({ children }: { children: React.ReactNode }) {
  ...
  return <Context.Provider value={{ consent, saveConsent, openPreferences }}>{children}{loaded && (!consent || customizing) && <View style={s.banner} accessibilityRole="alert"><Text style={s.copy}>PLACEHOLDER LEGAL COPY — pending legal review. We use necessary storage to operate this site. Optional analytics and marketing are off until you choose.</Text>{customizing && <><PressableScale onPress={() => setAnalytics(v => !v)}><Text style={s.choice}>{analytics ? '✓' : '○'} Analytics</Text></PressableScale><PressableScale onPress={() => setMarketing(v => !v)}><Text style={s.choice}>{marketing ? '✓' : '○'} Marketing</Text></PressableScale></>}<View style={s.actions}><PressableScale onPress={() => setCustomizing(v => !v)}><Text style={s.link}>{customizing ? 'Close' : 'Customize'}</Text></PressableScale><PressableScale onPress={() => saveConsent({ analytics: false, marketing: false })}><Text style={s.link}>Necessary only</Text></PressableScale><PressableScale onPress={() => saveConsent(customizing ? { analytics, marketing } : { analytics: true, marketing: true })}><Text style={s.accept}>{customizing ? 'Save choices' : 'Accept all'}</Text></PressableScale></View></View>}</Context.Provider>;
}
