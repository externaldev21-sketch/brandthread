...
const required = process.env.NATIVE_SELLER_DASHBOARD_REQUIRED === '1';
...
async function swipeDashboard() {
  ...
  if (before.height !== after.height) {
    throw new Error('Seller dashboard scroll viewport changed size during the iOS swipe');
  }
}
...
try {
  ...
  if (String(capabilities.platformName).toLowerCase() !== 'ios') {
    throw new Error('The seller dashboard native interaction check requires iOS');
  }
  ...
  console.log('Verified iOS dashboard scrolling, Studio overlay interactions, and seller create routes.');
} finally {
